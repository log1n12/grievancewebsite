...
if (isset($_POST['loginButton'])) :
    ...
    $loginquery = "SELECT * FROM account WHERE username = :uname AND user_password= :upass AND acc_isActive = 'yes'";
    ...
    if ($count > 0) {
        ...
        } elseif ($admintype == "barangay_captain") {
            setcookie('id', $id, time() + (60 * 60 * 24 * 365), '/');
...
if (isset($_COOKIE['id'])) {
    ...
    foreach ($data as $row) {
        ...
        if ($userType == "barangay_admin") {
            ...
            header("location:dilg-dashboard.page.php");
        } elseif ($userType == "barangay_captain") {
            header("location:barangay-captain-dashboard.page.php");
        }
    }
}

if (isset($_POST['givePassword'])) {
    $uname = $_POST['uname'];
    $uemail = $_POST['uemail'];
    $ubrgy = $_POST['ubrgy'];

    $pendingcountsql = "SELECT * FROM account WHERE username = '$uname' AND sec_email = '$uemail' AND barangay = '$ubrgy'";
    $pendingcountstmt = $con->prepare($pendingcountsql);
    $pendingcountstmt->execute();
    $pendingcount = $pendingcountstmt->rowCount();

    if ($pendingcount > 0) {
        $pendingque = $con->query($pendingcountsql);
        foreach ($pendingque as $row) {
            $id = $row['id'];
        }
        ...
        $updatepass = "UPDATE account SET user_password = :pass WHERE id = :id";
        $updatepassSTMT = $con->prepare($updatepass);
        if ($updatepassSTMT->execute([
            ...
            $mailbody =  "Update your new password as soon as you get your random password. Your new temporary password is " . $newpassword;
            $useremail = $uemail;
            require_once 'sendEmail.php';

            $confirm = "yes";
            $message = "We have sent your new password to your email. Please check your inbox and change it right away. Thank you.";
        }
    } else {
        $message = "The account information doesn't matched. Please check and try again.";
    }
}
